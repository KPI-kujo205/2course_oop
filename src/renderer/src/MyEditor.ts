...
class MyEditor {
  ...
  private configureToolbar() {
    this.toolbarButtons = [
      ...
      '#cube-btn',
      '#lineOO-btn'
    ].map((btn) => document.querySelector(btn) as HTMLInputElement)

    this.toolbarButtons[0].addEventListener('click', () => {
      this.currentShape = new Ellipsis(
        new MouseEvent('click'),
        this.ctx,
        this.fillColor,
        this.outlineColor
